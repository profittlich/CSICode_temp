...
{
public:
    virtual string GetName() override { return "TrackRecordArmDisplay"; }

    virtual void RequestUpdate(ActionContext *context) override
    {
        if (MediaTrack *track = context->GetTrack())
        {
            double state = DAW::GetMediaTrackInfo_Value(track, "I_RECARM");

            if (state > 0.5)
                context->UpdateWidgetValue(string("REC"));
            else
                context->UpdateWidgetValue(string(""));
        }
        else
            context->ClearWidget();
    }
